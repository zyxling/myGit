...
<body>
	...
	*/
	...
	function build_page_nav(result){
	    ...
	    var nextPageLi = var prePageLi = $("<li></li>").append($("<a></a>").append("&raquo;"));
	    ...
	    $.each(result.extend.pageInfo.navigatepageNums,function(index,item)){
	        var numLi = var prePageLi = $("<li></li>").append($("<a></a>").append(item));
	...
	function validate_add_form(){
	    //1.拿到要校验的数据，使用正则表达式，查询jequry文档
	    var empName = $("#empName_add_input").val();
	    //两个正则表达式用|（或）连接
	    var regName = //;
	    /**
	    测试数据
	    alter(regName.test(empName));
	    return false;
	    */
	...
	//为编辑绑定单击事件
	//1.由于在创建按钮之前绑定了click事件，所以无法绑定
	//$(".edit_btn").click(function(){
	    //alter("edit");
	//});
	//2.jquery文档中有live（）方法，可以在按钮创建之前绑定单击事件，但jQuery新版本中live方法被删除了，是用on方法替代了
